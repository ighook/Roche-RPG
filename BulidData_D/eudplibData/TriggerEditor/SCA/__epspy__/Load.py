...
# (Line 6) const loadItem = PVariable();
loadItem = _CGFW(lambda: [PVariable()], 1)[0]
# (Line 8) function LoadSlot() {
@EUDTracedFunc
def LoadSlot():
    # (Line 9) const cp = getcurpl();
    EUDTraceLog(9)
    cp = f_getcurpl()
    # (Line 10) for(var i = 0; i < 5; i++) {
    EUDTraceLog(10)
    i = EUDVariable()
    i << (0)
    _t1 = EUDWhile()
    ...
    if _t1(i >= 5, neg=True):
        def _t2():
            EUDTraceLog(10)
            i.__iadd__(1)
        # (Line 11) user.slotLevel[5 * cp + i] = data.userSlot[5 * cp + i] / 1000000;
        EUDTraceLog(11)
        _ARRW(user.slotLevel, 5 * cp + i) << (data.userSlot[5 * cp + i] // 1000000)
        # (Line 12) }
        # (Line 13) }
        EUDSetContinuePoint()
        _t2()
